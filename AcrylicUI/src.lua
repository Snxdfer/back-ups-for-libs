...
function AcrylicBlur:_Render(distance)
    ...
    Connections["blur_render"] = RunService.RenderStepped:Connect(Update)
    task.spawn(OnChange)
end
...
function AcrylicBlur:Destroy()
    ...
    local blur = Lighting:FindFirstChild("AcrylicBlurEffect")
    if blur then
        blur:Destroy()
    end
end
...
function Library:_SetupKeybindListener()
    Connections["keybind_listener"] = UserInputService.InputBegan:Connect(function(input, gameProcessed)
...
function Library:_CreateWindowControls()
    ...
    minimizeBtn.MouseEnter:Connect(function()
        CreateTween(minimizeBtn, {ImageColor3 = Colors.Text}, animationspeed.Fast)
    end)

    minimizeBtn.MouseLeave:Connect(function()
        CreateTween(minimizeBtn, {ImageColor3 = Colors.TextDark}, animationspeed.Fast)
    end)
    ...
    closeBtn.MouseLeave:Connect(function()
        CreateTween(closeBtn, {ImageColor3 = Colors.TextDark}, animationspeed.Fast)
    end)
...
function Library:_SetupSmartResize(handle)
    ...
    handle.MouseEnter:Connect(function()
        CreateTween(handle, {ImageColor3 = Colors.Text}, animationspeed.Fast)
    end)
...
function Library:SaveConfig(configName)
    ...
    local success, err = pcall(function()
        writefile("AcrylicConfigs/" .. configName .. ".json", HttpService:JSONEncode(configData))
    end)
...
function Library._SelectTab(lib, tab, btn, stroke, icon, textLabel, textGradient)
    ...
    if lib.currentTab then
        ...
        CreateTween(lib.currentTab.icon, {ImageColor3 = Colors.TextDark}, animationspeed.Fast)
    ...
    CreateTween(icon, {ImageColor3 = Colors.Text}, animationspeed.Fast)
...
function Library._CreateTextBox(tab, config)
    ...
    textBox.Focused:Connect(function()
        ...
        CreateTween(textBoxStroke, {Color = Colors.Accent}, animationspeed.Fast)
        CreateTween(icon, {ImageColor3 = Colors.Text}, animationspeed.Fast)
    end)

    textBox.FocusLost:Connect(function(enterPressed)
        CreateTween(textBoxContainer, {BackgroundTransparency = 0.04}, animationspeed.Fast)
        CreateTween(textBoxStroke, {Color = Colors.Border}, animationspeed.Fast)
        CreateTween(icon, {ImageColor3 = Colors.TextDark}, animationspeed.Fast)
